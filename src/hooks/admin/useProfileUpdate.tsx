...
export const useProfileUpdate = () => {
    ...
    const mutation = useMutation<any, any, any, unknown>({
        mutationFn: async (newTodo) => {
            return await axios.post(`${baseurl}adminpanel/profileupdates/`, newTodo, {
                headers: {
    ...
    const mutationUpdate = useMutation<any, any, any, unknown>({
        mutationFn: async (newTodo: any) => {
            return await axios.patch(`${baseurl}adminpanel/profileupdates/${vid}/`, newTodo, {
                headers: {
                    Authorization: `Bearer ${authToken?.access}`,
                     'Content-Type': 'multipart/form-data'
                }
    ...
    function handleKeyDownLoanId(e: React.KeyboardEvent<HTMLInputElement>) {
        ...
        const mutationUserInsert = useMutation<any, any, any, unknown>({
            mutationFn: async (newTodo: any) => {
                return await axios.get(`${baseurl}adminpanel/profileupdates/${vid}`, {
                    headers: {
                        Authorization: `Bearer ${authToken?.access}`,
                        'Content-Type': 'multipart/form-data'
                    }
